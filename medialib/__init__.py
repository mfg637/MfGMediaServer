...
class ItemCountCache:
    ...
    def get_items_count(
        ...
        expected_state = (
            ItemCountCache.State.TOTAL
            if tags_groups is None
            else ItemCountCache.State.TAGS
        )
        if expected_state is ItemCountCache.State.TAGS:
            tags_group_tulpified = tuple(tags_groups)
            if (
                self._state == expected_state
                and self._tags_groups == tags_group_tulpified
            ):
            ...
            self._number_of_items = medialib_db.files_by_tag_search.count_media_by_tags(
                db_connection,
                *tags_groups,
                filter_hidden=medialib_db.files_by_tag_search.HIDDEN_FILTERING(
                    hidden_filtering
                ),
            )
            return self._number_of_items
        elif (
            expected_state is ItemCountCache.State.TOTAL
    ...
    def count_pages(
        ...
        items_count = self.get_items_count(
            db_connection, hidden_filtering, tags_groups
        )
...
medialib_blueprint = flask.Blueprint(
    "medialib", __name__, url_prefix="/medialib"
)
...
def medialib_tag_search():
    ...
    order_by = int(
        flask.request.args.get(
            "sorting_order",
            medialib_db.files_by_tag_search.ORDERING_BY.RANDOM.value,
        )
    )
    hidden_filtering = int(
        flask.request.args.get(
            "hidden_filtering",
    ...
    if check_empty_group(tags_count, tags_list, not_tag) or check_empty_tag(
        tags_count, tags_list
    ):
        ...
        tags_groups = [
            {
                "not": bool(int(not_tag[i])),
                "tags": [],
                "count": int(tags_count[i]),
            }
    ...
    if tags_groups is not None:
        ...
        title = "Search query results for {}".format(
            (
                " and ".join(
                    [
                        ("not " if tags_group["not"] else "")
                        + tags_group["group_str"]
                        for tags_group in tags_groups
                    ]
                )
            )
...
def register_tags():
    def discard_existing_tags():
        existing_tags_categorised = medialib_db.get_tags_by_content_id(
            content_id
        )
        existing_tag_ids = []
    ...
    if content_id is None:
        ...
        if db_content is not None:
            ...
            content_new_data = {
                ...
                "addition_date": datetime.datetime.fromtimestamp(
                    path.stat().st_mtime
                ),
                "content_id": None,
                "origin_name": None,
...
def medialib_show_duplicates():
    ...
    duplicated_group_items = medialib_db.find_duplicates(
        connection=db_connection, show_alternates=show_alternates
    )
...
@medialib_blueprint.route(
    "/db-drop-thumbnails/<int:content_id>", methods=["GET"]
)
@shared_code.login_validation
...
def load_representations(
    content_id: int, file_path: pathlib.Path, db_connection
) -> list[medialib_db.srs_indexer.ContentRepresentationUnit]:
    representations = medialib_db.get_representation_by_content_id(
        content_id, db_connection
    )
    if len(representations) == 0:
        logger.debug(
            "register representations for content id = {}".format(content_id)
        )
        cursor = db_connection.cursor()
        medialib_db.srs_indexer.srs_update_representations(
            content_id, file_path, cursor
        )
...
def complex_formats_processing(
    img, file_path=None, allow_origin=None
) -> PIL.Image.Image:
...
@medialib_blueprint.route(
    "/thumbnail/<string:_format>/<int:width>x<int:height>/id<int:content_id>"
)
@shared_code.login_validation
def gen_thumbnail(
    _format: str, width: int, height: int, content_id: int | None
):
    ...
    if config.thumbnail_cache_dir is not None:
        thumbnail_file_path, thumbnail_format = (
            medialib_db.get_thumbnail_by_content_id(
    ...
    file_path = shared_code.root_dir.joinpath(content_metadata.file_path)
    compatibility_level = int(flask.request.cookies.get("clevel", "-1"))
    if compatibility_level == -1:
        raise ValueError("Unexpected value of compatibility level")

    img = None
    allow_hashing = True

    if content_metadata.content_type == "image":
        ...
        if allow_origin:
            ...
            if representations[-1].format == "jxl":
                jpeg_buffer = io.BytesIO(
                    jpeg_xl_fast_decode(representations[-1].file_path)
                )
                return flask.send_file(jpeg_buffer, mimetype="image/jpeg")
            logger.info(
                "generate thumbnail from best available representation"
            )
    ...
    if file_path.suffix == ".jxl":
        ...
        if (
            allow_origin
            and _format == "avif"
            and check_max_size(img.width, compatibility_level)
            and check_max_size(img.height, compatibility_level)
        ):
    ...
    if allow_hashing:
        existing_image_hash = medialib_db.get_image_hash(
            content_id, db_connection
        )
    ...
    if config.thumbnail_cache_dir is not None:
        ...
        if thumbnail_file_name is not None:
            thumbnail_file_path = pathlib.Path(
                config.thumbnail_cache_dir
            ).joinpath(thumbnail_file_name)
...
def post_tags():
    ...
    db_connection = medialib_db.common.make_connection()
    medialib_db.add_tags_for_content_by_tag_ids(
        data["content_id"], data["tag_ids"], db_connection
    )
    db_connection.close()
    return "OK"
...
def compare_image():
    ...
    db_connection = medialib_db.common.make_connection()
    content_id_list = medialib_db.find_content_by_hash(
        value_hash, hue_hash, saturation_hash, db_connection
    )
    ...
    for first_index in range(len(image_data_list)):
        for second_index in range(first_index + 1, len(image_data_list)):
            ...
            compare_result = CompareResult(
                ...
                size_equal,
                first_image_data.calc_aspect_ratio()
                == second_image_data.calc_aspect_ratio(),
                first_image_data.calc_size() > second_image_data.calc_size(),
                first_image_data.download_date
                > second_image_data.download_date,
                first_image_data.source == second_image_data.source,
                first_image_data.alternate_version == True
                and second_image_data.alternate_version == True,
                no_difference,
                difference,
            )
...
def mark_alternate():
    ...
    connection = medialib_db.common.make_connection()
    medialib_db.mark_alternate_version(
        content_id_list[0], content_id_list[1], connection
    )
    connection.close()
    return "OK"
