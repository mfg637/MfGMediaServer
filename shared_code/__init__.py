    ...
    return [
        {
            ...
            "name": sort_order.name.lower().replace("_", " "),
        }
...
def login_validation(func: Callable[P, R]) -> Callable[P, R]:
...
def extract_frame_from_video(
    img: pyimglib.decoders.frames_stream.FramesStream,
):
...
def jpeg_xl_fast_decode(file_path: pathlib.Path) -> bytes:
    commandline = ["djxl", file_path, "-", "--output_format", "jpeg"]
